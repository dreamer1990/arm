    ...
    while(1)
    {
        ...
        else
        {
            if (FD_ISSET(newfd, &readfds)) 
            {
                ...
                ret = recvfrom(newfd, buf, SIZE, 0, NULL, NULL);
                if (ret <= 0)
                    break;
                len = ret;
                crc = crc16(0x88, buf, ret - 2);
                printf("=====> len = %d %p  %p\n", len, buf[ret - 2], buf[ret - 1]);
                if (crc == (unsigned short)((buf[ret - 2] << 8) | buf[ret - 1])) 
                {
                    buf[ret - 2] = '\0';
            ...
            if (FD_ISSET(STDIN_FILENO, &readfds))
            {
                ...
                buf[len] = (crc >> 8) & 0xff;
                buf[len + 1] = crc & 0xff;
                ret = sendto(newfd, buf, len + 2, 0, NULL, 0);
                if (ret <= 0)
                    break;
                printf("send  %d bytes\n", ret);
